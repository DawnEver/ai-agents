...
> **发布期改动（脚本分页卡）**：小红书主体轮图现在由发布脚本 `gen_xhs_pages.py` 从正文自动渲染成**分页文字卡**（第 1 张即标题卡/封面），详见 `.claude/skills/post-publish/_platforms/xiaohongshu.md`。因此：
> - **封面无需再单独 AI 生图**——标题卡由脚本用 H1 渲染。下面的封面规范仅在你想额外做一张 AI 封面时参考。
> - 本节的「正文配图」清单改为**可选的 AI 补充图**（真实截图 / 对比图），穿插在文字卡之间，不再是主体。写作时正文照常纯文字即可，`images.md` 的配图清单可空或只列补充图。
