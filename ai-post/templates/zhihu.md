...
4. **机制拆解 / 技术深挖 (核心章节之一)** — Dig into *how it actually works*, not just what it does. This is co-core with the comparison for 知乎's technical audience. Pick the 1-3 most interesting mechanisms (architecture, key algorithm, clever design trade-off) and walk through the **logic** so the reader understands *why* it works. Ground your own understanding in `source-exploration.md` / `source-analysis.md` Standout Details (for a research report, the mechanism is the argument/method, not code), but **explain in plain terms — do NOT cite source file paths, function names, or line numbers in the article**; readers want the idea, not a code map. A short, self-contained code snippet (a few lines) is welcome when it makes the mechanism concrete. A reader should finish this section able to explain the design to a colleague. Don't hand-wave with feature lists — show the actual mechanism.
