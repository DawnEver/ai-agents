...
> - 不要一个一个 `await` 串着跑。
> - **速度档位(在 verdict 里记录用了哪档)**：默认三方(Opus+DeepSeek+Codex)；`--fast` 去掉最慢的 Codex(Opus+DeepSeek)；**急用可降级为单模型 inline 自审**——但这已不是真三方,verdict 顶部必须标注 `reviewer: single-model inline` 让读者知道置信度打了折。
