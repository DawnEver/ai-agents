---
name: post-review
description: 三方会审 — each reviewer identity is run independently by three heterogeneous models (Opus + DeepSeek + Codex) via fabric MCP fan-out + sharp-review's merge engine. Disagreements between models surface genuine issues.
...
身份B 技术核查员:  [Opus] [DeepSeek V4 Pro] [Codex]  → fabric MCP fan-out → merge-findings → merged findings
                              ↓
...
| Phase 1-2 | `03-execution.md` | Per-identity fabric MCP fan-out + merge-findings collection |
