...
| Command | Description |
|---------|-------------|
| `/manuscript-review:new <pdf>` | Main pipeline. Re-invoking with a slug resumes from the latest artifact. |
| `/manuscript-review:rerun <step> <slug>` | Re-run any single step. |

## Pipeline

`/manuscript-review:new <pdf>` walks a 9-step pipeline with progressive disclosure — `SKILL.md` is the map, each sub-file under `.claude/skills/manuscript-review/` is the playbook:
...
```
.claude/
  skills/manuscript-review/  — pipeline (SKILL.md + 00–08 sub-files)
  skills/manuscript-rerun/   — re-run single step
