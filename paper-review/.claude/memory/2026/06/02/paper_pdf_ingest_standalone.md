...
description: paper_pdf_ingest extracted to standalone git repo at paper-review/paper_pdf_ingest/
metadata:
  type: project
---

`paper-review/paper_pdf_ingest/` is now a standalone git repo (independent `.git`, not tracked by paper-review).

**Why:** Library is reusable outside paper-review; needs its own release cycle, CI, and versioning.

**How to apply:**
- paper-review's `.gitignore` has `paper_pdf_ingest/` — it will never be re-added to paper-review tracking
- To work on the library, open `paper-review/paper_pdf_ingest/` as its own project in Claude Code
