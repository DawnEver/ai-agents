...
deletes data; it only adds missing ledger entries and regenerates derived
...
from literature_review.acquire.types import Attempt, DownloadRecord, Outcome
from literature_review.acquire.verify import safe_filename, sha256_file, validate_pdf
...
def repair_workspace(topic_dir: Path, dry_run: bool = False) -> dict[str, Any]:
    ...
    if pdf_dir.exists():
        ...
        if not dry_run:
            ...
            from literature_review.acquire import ledger as ledger_mod2
    ...
    if queue_json.exists() and verified:
        from literature_review.pipeline.acquire import manifest_rows, write_download_manifest
