...
def needs_enrichment(data: dict[str, Any]) -> bool:
    ...
    if title.lower().endswith(".pdf"):
        return True
    if not data.get("creators") and not extract_doi(data.get("DOI") or ""):
        return True
    return False
...
def plan_update(item: dict[str, Any], fetcher: Any = None) -> tuple[str, dict[str, Any] | None]:
    ...
    *fetcher* maps (kind, value) -> update dict; defaults to live HTTP.
...
def enrich_items(
    ...
    *only_keys* restricts enrichment to specific item keys (e.g. the workspace
    ...
    for item in items:
        ...
        except Exception as e:  # network hiccup on one item must not stop the run
...
def mirror_attachments(
    ...
    *only_keys* restricts mirroring to attachments whose parentItem is in the
    ...
    is available; any 200 with a non-empty body is accepted (attachments are
    ...
    for att in attachments:
        ...
        if only_keys is not None and data.get("parentItem") not in only_keys:
            continue
        if not data.get("md5"):  # never uploaded — nothing to mirror
