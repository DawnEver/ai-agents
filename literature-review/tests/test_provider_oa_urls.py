...
from literature_review.providers.semantic_scholar import (
    SEARCH_FIELDS,
    _open_access_pdf,
)


def test_search_fields_request_open_access_pdf():
...
def test_open_access_pdf_is_used_when_present():
    ...
    assert _open_access_pdf(record) == "https://repo.example/paper.pdf"


def test_falls_back_to_arxiv_pdf():
    record = {"openAccessPdf": None, "externalIds": {"ArXiv": "2401.00001"}}
    assert _open_access_pdf(record) == "https://arxiv.org/pdf/2401.00001"
...
def test_empty_when_no_open_access_copy():
    assert _open_access_pdf({"externalIds": {"DOI": "10.1/x"}}) == ""
    assert _open_access_pdf({}) == ""


def test_tolerates_malformed_payloads():
    assert _open_access_pdf({"openAccessPdf": "not-a-dict"}) == ""
    assert _open_access_pdf({"externalIds": "not-a-dict"}) == ""
