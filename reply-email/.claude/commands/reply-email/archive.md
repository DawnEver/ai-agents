...
All formats referenced here are defined once in `AGENTS.md` (Reference section) — do not
restate them. Step order mirrors AGENTS.md → Workflow step 7.

## Steps

1. Move `ongoing/<topic>/` → `archived/<YYYY>/<MM>/<DD>/<topic>/` with `mv`. If the same slug
   already archived today, apply the `-r<N>` suffix (AGENTS.md → Naming conventions).
2. Diff `draft.md` vs `final.md` **before renaming** and learn from it
   (AGENTS.md → Diff learning).
...
5. Write `meta.md` (AGENTS.md → meta.md schema): frontmatter + summary + `## Diff notes`.
